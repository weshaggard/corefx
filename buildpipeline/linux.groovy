...
simpleDockerNode('microsoft/dotnet-buildtools-prereqs:rhel7_prereqs_2') {
    ...
    stage ('Build Product') {
        sh "./build.sh /p:ArchGroup=x64 -${params.CGroup}"
    }
    stage ('Build Tests') {
        def additionalArgs = ''
        if (params.TestOuter) {
        ...
        sh "./eng/build-tests.sh --ci --preparemachine /p:ArchGroup=x64 /p:ConfigurationGroup=${params.CGroup} /p:SkipTests=true ${additionalArgs} /p:ArchiveTests=true /p:EnableDumpling=true"
    }
